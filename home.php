		<div class="row">
 			<div class="col-md-6 animated zoomInLeft" id="content-home" style="z-index: 500;">

 				<ul class="menu-home center-block">
 					<a href="" data-toggle="modal" data-target="#pilihdesain" data-remote="false">
 						<li class="bg-tosca shadow-bottom">
		 					<div class="left-menu">
	 							<img src="images/assets/desain.png" class="icon-menu img-responsive">
	 						</div>
	 						<div class="right-menu text-center">
	 						Make Your Gift		
	 						</div>
	 					</li>	
 					</a>
 					
 					<a href="index.php?page=galeri">
 						<li class="bg-red shadow-bottom">
		 					<div class="left-menu">
	 							<img src="images/assets/galeri.png" class="icon-menu img-responsive">
	 						</div>
	 						<div class="right-menu text-center">
	 						Gallery	
	 						</div>
	 					</li>	
 					</a>
 					
 					<a href="index.php?page=bantuan">
 						<li class="bg-navy shadow-bottom">
		 					<div class="left-menu">
	 							<img src="images/assets/bantuan.png" class="icon-menu img-responsive">
	 						</div>
	 						<div class="right-menu text-center">
	 						Help
	 						</div>
	 					</li>	
 					</a>
 					<a href="index.php?page=tentang">
 						<li class="bg-red shadow-bottom">
		 					<div class="left-menu">
	 							<img src="images/assets/tentang.png" class="icon-menu img-responsive">
	 						</div>
	 						<div class="right-menu text-center">
	 						About Us
	 						</div>
	 					</li>	
 					</a>
 				</ul>
 			</div>
 			<div class="col-md-6 animated zoomInRight" id="content-home">
 				<div class="box-video">
 					<iframe width="460" height="215" class="shadow-bottom " src="https://www.youtube.com/embed/m-uYZmOmKpc" frameborder="0" allowfullscreen></iframe>
 				</div>
 			</div>
 		</div>